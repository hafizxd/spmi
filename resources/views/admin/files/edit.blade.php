...
@section('content')
    ...
    <div class="row">
            ...
            <div class="card">
                <div class="row">
                    <div class="col-md-6 d-flex mt-2 p-0">
                        <h4>Edit File Untuk {{ request()->type }}</h4>
                    </div>
        ...
        <div class="col-sm-12">
            <div class="card">
                <form class="p-4" method="post" action="{{ route('admin.files.update', ['type' => request()->type, 'file' => $file->id]) }}" enctype="multipart/form-data">
                    @csrf
                    @method('PUT')
                    <div class="form-row">
                        <div class="col mb-4">
                            ...
                            <select name="cycle_id" id="cycle_id" class="form-control js-example-basic-single @error('cycle_id') is-invalid @enderror">
                                ...
                                @foreach ($cycles as $value)
                                    <option value="{{ $value->id }}" @if($file->cycle_id == $value->id) selected @endif>{{ $value->order_no }} - {{ $value->year }}</option>
                                @endforeach
                            </select>
                            @error('cycle_id')
                        ...
                        <div class="col mb-4">
                            ...
                            <input type="text" class="form-control @error('title') is-invalid @enderror" id="title" name="title" value="{{ old('title', $file->title) }}" placeholder="Judul File">
                            @error('title')
                                <div class="invalid-feedback">
                                    {{ $message }}
                                </div>
                        ...
                        <div class="col mb-4">
                            ...
                            <a target="_blank" href="{{ asset("storage/attachment/".$file->attachment) }}">{{ $file->attachment }}</a>
